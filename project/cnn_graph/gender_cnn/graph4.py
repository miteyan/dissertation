...
def create_datasets(input_folder, num_nodes):
    ...
    return np.array(centrality), np.array(subgraphs), np.array(labels)
...
graphs, subgraphs, labels = create_datasets(FOLDER, MEDIAN)
# np.savetxt("./data/graphs.npy", graphs, fmt='%1.17f')
# np.save("./data/graphs2.npy", subgraphs)
...
L = [graph.laplacian(A, normalized=True) for A in subgraphs]
...
if True:
    ...
    model.fit2(subgraphs, labels, subgraphs, labels)
    # change train from signal to a graph
    model_perf.test2(model, name, params, train_data, train_labels, val_data, val_labels, test_data, test_labels)
...
Word Count:         & \bf 11464\footnotemark[1]\\
...
\section{Motivation}
Machine-learning techniques typically use structured, organised data, which restricts the domain of usable data. Unstructured data , such as graphs, does not have a pre-defined data model and arises commonly, for example in social networks, web networks and bioinformatics. Consequently, graph classification algorithms would allow access to vast amounts of currently unusable unstructured data.
An interesting problem is the classification of \textit{mobility graphs}, where the nodes correspond to the locations visited by the user and edges to transitions between these locations. 
This could be used for early detection of Alzheimer’s Disease from alterations in the movement patterns of its sufferers. The prominent prevalence of mobile phone usage allows us to ethically monitor and analyse users' locations by solely using local computations on the users' smartphones. 
On-device machine learning has massive potential in the mobile domain. For example, optimising battery life through intelligent task management utilising usage patterns of the users applications~\cite{androido}, or to detect user well-being~\cite{well-being}. Furthermore, an external centralised server for data processing is no longer needed, eliminating concerns regarding the leaking of sensitive data through the network or through security flaws and also reducing data protection legislation concerns.
\section{This Project}
This dissertation details the development of an Android application that runs all stages involved in the inference of the users' occupation class, with all required computations taking place solely on their smartphone. Mobility graphs extracted from spatio-temporal datasets are used to train a collection of supervised machine-learning algorithms, including a new Convolutional Neural Network architecture~\cite{CNN1}. The best model, Random Forest, is trained and transferred onto the application.
The mobility graphs used for training are constructed from datasets containing user's demographic labels such as gender or occupation. The classifiers are optimised and evaluated to determine the best performing model to export to the application. The application collects and clusters the location data into graphs, which are later used to infer the users' demographic class, their occupation type. This project has the potential to impact the design of future applications for location tracking and classification analysis. \\
\section{Related Work}
Smartphones already use machine-learning locally to do various tasks such as predictive typing on keyboards, classifying images into folders and making suggested actions of highlighted text~\cite{androido}. Wearable technology like smartwatches also make use of passive data collected from the sensors throughout the day to infer step counts, heart rate or even if the user could have an underlying disease~\cite{apple}.
...
\chapter{Preparation}
My preparation consisted of studying about supervised machine-learning algorithms in order to optimise them for the task of mobility-graphs classification. Understanding techniques for evaluating the machine-learning algorithms was paramount in ensuring that the correct algorithm was chosen. Research in graph-theory was done to find the most relevant features that could be used to classify graphs and to find techniques such as clustering to create the graphs from the raw location data. This chapter details the algorithms used for clustering, graph feature extraction, supervised classification algorithms and the software engineering practices used. 
\section{Clustering}
Location data needs to be grouped so that nearby location recordings are in the same cluster, which represents a single place the user has visited. Clustering will reduce the number of locations visited in the graph and increase the edge weight between the nodes. That makes the graphs simpler to process whilst conveying more information through greater edge weights than a graph simply representing each location as a node. 
...
Naive Bayes classifier is a simple probabilistic classifier based on applying Bayes' theorem with the assumption that the features are independent of each other. This is the major problem with this approach, since features derived from the graphs are likely dependent on each other. An advantage of Naive Bayes is that it only requires a small amount of training data to estimate the parameters necessary for classification.
Bayes' theorem describes the likelihood of an event given another event, for which the prior probabilities are known.
...
\end{split}
\end{equation}
Finally, to construct the naive Bayes classifier the most probable hypothesis is chosen known as the maximum a posteriori decision rule. The corresponding Bayes classifier is the function that assigns a class label $\hat y = C_k$, for some class $C_k$ as:
\begin{equation}
\begin{split}
...
\end{figure}
The Sigmoid function (Figure \ref{fig:sigmoid-tanh}) is commonly used in neural networks. 
...
The goal of backpropagation is to find a set of weights for the entire multi-layered perceptron network that minimises the error term using the gradient descent algorithm. To do this we need to find  $\frac{\partial E(\textbf{w})}{\partial \mathbf{w}}$ for the entire network which needs the calculation of $\frac{\partial E(\textbf{w})}{\partial {w}_{i,j}}$, for each weight ${w}_{i,j}$ in the network. The total error in the network produced over the entire training set is now defined as $E(\textbf{w}) = \sum_{i=1}^{n} E_i(\mathbf{w})$ and $\frac{\partial E(\textbf{w})}{\partial \mathbf{w}} =  \sum_{i=1}^{n} 
...
The overall process involves initialising the filters and weights with random values and then feeding the input data forward through the network using the convolution, ReLU, pooling and fully connected operations. The error is then calculated between the target class and predicted class with a loss function. Backpropagation is used to update all the filters and weight values to minimise the error for subsequent training steps. This is repeated for all of the training examples and allows the network to generalise well towards new unseen examples. The size of the layers, parameters and filters are fixed from the start so to find out the best combinations an additional validation set can be used to optimise for the best test accuracy. 
...
\textit{Activities} are used to interact with the user. The Activity class creates a window for the UI and be embedded inside of another activity, or can be a full-screen or floating window. Most subclasses of the Activity class implement \textit{onCreate} and \textit{onPause} methods to initialise the activity and deal with the user leaving the activity.
\subsection{View}
...
\subsection{Intent}
\textit{Intents} are a passive data structure holding an abstract description of an action to be performed. They can be used to launch Activities, start Services, communicate with Services, etc.
\subsection{Services}
...
\subsection{Alarm}
\textit{Alarms} allow the application to be run at a specified times. An Intent is registered with the alarm so that when the alarm goes of the Intent can be run which can be used to start a task, service or a target application.
...
Development work was done on a MacBook running Mac OS 10.12. Training and testing the algorithms were done on a remote server provided by the Systems Research Group to ensure no sensitive data is leaked out of the server it resides on. I used a OnePlus 5T Android smartphone with Android Oreo to test the application.
\subsection{Ethics}
...
I used PyCharm for building and testing the Python code since it contains a SSH server interface for accessing the dataset stored on its private server, preventing sensitive information from data breaches.\\
\subsection{Languages}
The machine-learning libraries and NetworkX are in Python, allowing me to quickly experiment with different techniques. It was unclear if graph-feature extraction would need to be ported to the Android application since the graph CNN algorithm would not have required this. The Android application uses Java. Thus, Weka and JUNG Java libraries were used \cite{WEKA, JUNG}.
...
I was provided an Android application developed by the SRG group as part of a research project that collected GPS data every 30 seconds in the background. This was modified to prevent data being sent to the server, to allow clustering, graph-edge list formation, and run the classifier locally on the mobile phone..\\
...
Figure 2.7 contains an overview of the libraries used, the purpose and the licence for each.
\begin{figure}[H]
\begin{center}
...
\begin{itemize}
\item Training, optimising, and evaluating the performance of the supervised machine-learning algorithms on mobility graphs.
...
\item  Evaluating the models quantitatively, then training and porting the best performing of these onto the Android application.
...
In this chapter I summarise the implementation of the data preprocessing, graph constructions, training, comparison of the machine-learning algorithms, the training and porting of the best performing algorithm onto an Android application.
...
\begin{figure}[H]
\centering
\includegraphics[scale=0.14]{sysdiagram.png}
...
Table \ref{table:datasets} describes the datasets, preprocessing and classes inferred using the models. The datasets comprised of time and latitude-longitude pairs for each user. Users whose graphs were disconnected once clustered were discarded, since not all graph features chosen are defined on disconnected graphs. Disconnected graphs can happen because when the user has not recorded location data consistently, creating large discrepancies between location recordings creating partitions across the clusters.
\begin{table}[H]
...
I modify the Erdos-Renyi random graph algorithm \cite{erdos}, which is used to create random graphs with a known probability distribution. I chose to create graphs of 50 nodes to give the classifiers more information to work whilst minimising the feature extraction and training times. 
\paragraph{The Erdos-Renyi Graph Generation Algorithm}


Erdos-Renyi algorithm, shown in Algorithm \ref{alg:erdos}, is used to create random graphs. An edge is created between two nodes randomly with the Bernoulli distribution with probability \textit{probEdge}. A simple modification allows an edge weight to be up to \textit{maxWeight}, as seen in Algorithm \ref{alg:erdos}. 
...
Clustering is used as a pre-processing step before constructing the graphs by grouping together locations from the dataset to form the nodes of the graph. Clusters consist of close-together locations to signify a visited region. Using the start and end times the user was at each cluster, the transitions between the clusters can be calculated to create the edge lists. I implemented the DBSCAN algorithm in Java so it also worked on the Android application. The parameters that worked for the DBSCAN in practice were 5 metres for the \textit{minDistance} and 5 minimum nodes needed to form a cluster. This ensures that any large building the user is in, such as at school or work, can be represented as a single node. 
\subsection{Graph Feature Extraction}
The construction of feature vectors for supervised machine learning dealt of using two libraries for retrieving the features defined in Section 2.2.1. Initially I used the Python library NetworkX to extract the same features; however when porting to the Android Application there was no corresponding library and so Java library JUNG was chosen due to its ability to construct the same feature vectors. These were tested to be equal by using the same edge lists and comparing the feature vectors composed by both. \\ 
...
In the following section I detail the techniques I used to optimise the different models. 
...
Random search needs a range of values each hyperparameter can take and the number of iterations to evaluate these parameters. This is a good choice when there can be a large range of values that are not intuitive to think about. I used this technique for the Neural Network implementation as there are so many values for the possible size and number of layers.
\subsection{Nested Cross Validation}
Nested cross validation~\cite{nestedcrossvalidation} is used to train and test the classification algorithms with the entire dataset.  This maximises the usage of the dataset whilst reducing selection bias. It involves an outer loop to split the data into training and test sets, and the inner loop is used to find the best hyperparameters of the model. Then the model is fit to these hyperparameters and then evaluated using the independent unseen test fold created by the outer loop. At the end, the hyperparameters for the algorithm will be known as well as the metrics such as accuracy and loss associated with using these hyperparameters. Figure \ref{fig:nestedcv} shows the cross-validation used in the project.
\graphicspath{{images/}}
\begin{figure}[H]
\centering
...
\textit{Maximum depth} of the tree which is used to control the size of the tree to prevent overfitting. \textit{Maximum features} is used to control the number of features from the original training set to use in the tree and are randomly selected and tried to get the best combination of features. \textit{Minimum  leaf samples} is used to control the number of samples at a leaf node. A small number will usually mean the tree is overfitted to the training data, whereas a large number prevents the tree from learning the data optimally.
\paragraph{Random Forests} Additionally to the decision tree parameters, the Random Forests also used a parameter for the number of trees in the forest.
...
An \textit{Activity} containing the button for inferencing is started when the application is opened. A button is used to start an \textit{Intent} which allows the inference to take place. The inference process starts by constructing the graph feature vector. The DBSCAN algorithm is applied to the location-time data collected to form the nodes. The edge list is created using the timestamps associated with the first and last value of time for the clusters. The graph features are then extracted from the edge list and the features vector is created. 


The feature vector can be fed into the Random Forest classifier to infer the class. The pre-trained Random Forest model is de-serialised from its stored location on the device and the users' mobility-graph feature-vector is used to infer the users' class. The resulting class information is shown on a new \textit{View} on the device for the user to analyse. 
\chapter{Evaluation}
In this chapter, I describe the metrics used to evaluate the algorithms and the results obtained. I present the performance of the classifiers used. I also discuss the performance of the Android application.
\section{Success Criteria}
Referring back to the success requirements set out in the project proposal:
\begin{itemize}
\item \textit{Training, optimising, and evaluating the performance of the supervised machine-learning algorithms on mobility graphs.}\\
...
The CNN algorithm for graphs was applied successfully, and highlighted the  performance differences with respect to the other classifiers. The evaluation of this is described in Section 4.3. 
\item  \textit{Evaluating the models quantitatively, then training and porting the best performing of these onto the Android application.}\\
...
The application has been tested to successfully work as described in Section 4.4. It is functional as it can apply the Random Forest algorithm on the feature vector extracted from the constructed mobility graphs locally on the device.
\end{itemize}
...
We can use a hypothesis test to deem a classifier to be statistically different from another. We propose a null hypothesis which states that there is no difference between the two classifiers, along with an alternative hypothesis which states there is a difference between the two. We also select a significance level, which is a probability threshold below which the null hypothesis will be rejected. Using a 2 sample t-test we can compare  two classifiers in a pair-wise method by determining the probability that one classifier is outperforming the other. If the probability is smaller than the significance level we can reject the null hypothesis as this indicates that this is an unlikely event. 
\end{itemize}
\section{Results}
...
\end{table}
\end{center}
Results show that the SVM performs the best for the data from the datasets. However, the F1 score is lower than the Java NN as it predicts a single class more often. \\
...
\end{table}
\end{center}
The Random Forest classifier performs the best consistently across the accuracy metrics. Random forests are constructed by combining several different independent classifiers, which identifies the best features to use from the random subset of available features. Since we chose the features from Section 2.2.1 manually, not all of the features could be useful for the classification of graphs therefore the random forest algorithm would have been able to discard these and used the best features to use for the classification of the graphs. Random forests also do not overfit to the data as much as decision trees, which perhaps explain the difference in performance between the two.

The SVM and Neural Networks performed similarly well and were able to infer the hypothesis function to map the graph features to the class. It is understandable the TensorFlow implementation did better than the Java NN as it implements more complex techniques to maximise its classification effectiveness; the full comparison can be seen in Section 4.3.3.
...
\end{table}
As seen in Table \ref{table:results4p}, the TensorFlow model performed statistically better than the Java implementation due to differences in the technical complexity in between the two implementations.
\graphicspath{{images/}}
\begin{figure}[H]
\centering
...
\subsection{Result}
Figure 4.3 shows the screen-shots of the Android application to infer the users' occupation class. Since its primary use is as a background service to make inferences based on the users' location patterns the front-end user interface is not of concern.
\begin{figure}[H]
\centering
\graphicspath{{images/}}
...
Figure 4.5 shows that the memory usage is less than 96MB. Given that most recent Android phones have at least 1GB of RAM the application should work on a whole range of phones. A large amount of garbage collection is seen after the edge list has been used, after creating the edge features and after the creating the WEKA feature vector. The de-serialisation of the random forest model also uses a lot of the memory utilised.
...
The project provides a novel framework for on-device machine learning based on mobility graphs constructed. This can be extended by simply replacing the classifier to infer other labels to solve different problems. As phones become more powerful and employ dedicated chips for AI tasks, we will need new types of application frameworks to exploit mobile sensor data to solve a new class of problems. This project successfully demonstrates the potential of on-device mobility-graphs classification.
...
Further work would go towards improving the quality of the data to increase the accuracies of the models. These improvements could come from adding more information to the mobility graphs since a lot of data is removed in the clustering stage. The time information and rare visits are also ignored when the locations are clustered.
...
\bibitem{WEKA} WEKA library source. 24/4/2018.
https://www.cs.waikato.ac.nz/ml/weka/
\bibitem{JUNG} JUNG library source. 24/4/2018. http://jung.sourceforge.net/
\bibitem{tensorflow}
...
\begin{enumerate}
\item Applied, optimised and tested the graph CNN algorithm to the location graphs constructed from raw location data to classify the user into their demographic class\cite{CNN1}.
\item Trained, optimised, and benchmarked the performance of the supervised machine learning algorithms on features extracted from mobility graphs.
...
- Continue work on the graph CNN and train, optimise, benchmark, evaluate.\\

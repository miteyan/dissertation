...
DBSCAN is a density-based clustering algorithm: given a set of points in some space, it groups together points with many neighbours in a cluster, and marks distant points with neighbours too far away as noise.

Given two parameters, a minimum distance $\epsilon$ that a point needs to be from another point to be considered its neighbour and the minimum number of points needed to form a cluster, minPoints.
\begin{enumerate}
\item Begin by picking an arbitrary point. \item 
If there are more than minPoints points within $\epsilon$ from that point, a cluster is formed. The cluster is expanded by checking the new points and if they have more than minPoints points within $\epsilon$ the cluster is grown recursively. 
\item Repeat the process once the cluster cannot be expanded.
\item A point with fewer than minPoints is considered as an outlier or "noise point" and does not belong to any cluster.
\end{enumerate}
...
\begin{itemize}
\item Graph Convolutional Neural Network algorithm is applied to the location graphs constructed from raw location data to classify the user into their demographic class.
\item Trained, optimized, and benchmarked the performance of the supervised machine
learning algorithms on features extracted from mobility graphs.
\item  Evaluated the models quantitatively and chosen the most appropriate of these and
pre-train the model and port into the Android application.
\item  Have an Android application which can construct mobility graphs from the users
location and infer the demographic class of the user locally using the model.
\end{itemize}
The 
...
I opted for TensorFlow and scikit-learn to assist in the development of the machine learning algorithms since they both contain efficient open source implementations of the algorithms. 
\textbf{Graphs}\\
I opted for NetworkX, an open source graph manipulation library to assist in the creation of graphs. The key reasons are that it contains many graph algorithms that are tested and efficiently implemented allowing for fast prototyping and creation of graphs. 

\textbf{Languages}\\
The machine learning libraries and NetworkX  both had Python API's and was exported to the Android application which uses Java.

\subsection{Starting Point}

The implementation of the Convolutional Neural Networks on Graphs with Fast Localized Spectral Filtering was available and applied to the graphs built.
